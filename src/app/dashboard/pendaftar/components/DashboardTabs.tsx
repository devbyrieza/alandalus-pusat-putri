...
export default function DashboardTabs({ statusProses }: DashboardTabsProps) {
  ...
  return (
    <div className="w-full bg-white  sticky top-0 z-30 border-b border-gold-100 shadow-sm overflow-hidden rounded-b-[2rem]">
        ...
        {tabs.map((tab) => {
          ...
          if (!isAccessible) {
            return (
              <div
                key={tab.id}
                className="flex-shrink-0 flex items-center gap-2 px-4 py-3 rounded-xl text-ink-400 bg-surface-50 border border-transparent opacity-60 cursor-not-allowed group relative"
              >
                ...
                <div className="absolute top-full left-0 mt-2 p-2 bg-ink-900 text-white text-[8px] rounded shadow-lg invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all z-50 whitespace-nowrap">
          ...
          return (
            <Link
              key={tab.id}
              href={tab.href}
              className={`flex-shrink-0 flex items-center gap-2 px-4 py-3 rounded-xl transition-all duration-300 relative group ${
                isActive
