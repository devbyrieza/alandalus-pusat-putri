...
export default function KepribadianTestPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 pb-32">
      ...
      {pendaftarId && (
        <div className="mb-4 p-4 bg-amber-100 border border-amber-300 rounded-xl text-amber-900 flex items-center gap-3 whitespace-nowrap inline-block shrink-0">
          ...
          <div key={q.id} className="bg-white p-5 rounded-xl border shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3">
              <div
                ...
                className={`cursor-pointer p-5 border-2 rounded-xl transition-all duration-300 ${answers[q.id] === "A" ? "bg-primary-50 border-primary-500 text-primary-950 shadow-md ring-4 ring-primary-500/5" : "border-stone-100 hover:border-primary-200 hover:bg-slate-50"}`}
              >
                <span
                  className={`font-black mr-3 border rounded-xl px-2.5 py-1 text-xs transition-colors ${answers[q.id] === "A" ? "bg-primary-600 text-white border-primary-600" : "bg-white text-stone-400 border-stone-200"}`}
                >
                  A
                </span>
              ...
              <div
                ...
                className={`cursor-pointer p-5 border-2 rounded-xl transition-all duration-300 ${answers[q.id] === "B" ? "bg-primary-50 border-primary-500 text-primary-950 shadow-md ring-4 ring-primary-500/5" : "border-stone-100 hover:border-primary-200 hover:bg-slate-50"}`}
              >
                <span
                  className={`font-black mr-3 border rounded-xl px-2.5 py-1 text-xs transition-colors ${answers[q.id] === "B" ? "bg-primary-600 text-white border-primary-600" : "bg-white text-stone-400 border-stone-200"}`}
                >
                  B
                </span>
                <span className="font-bold text-sm sm:text-base">
      ...
      <div className="sticky bottom-4 z-50 flex gap-3">
        ...
        {page < totalPages - 1 ? (
          <button
            onClick={handleNext}
            className="flex-1 py-4 bg-primary-600 hover:bg-primary-700 text-white font-black uppercase tracking-widest text-sm rounded-xl shadow-lg shadow-primary-200 transition-all active:scale-95"
          >
            Lanjutkan
          </button>
        ) : (
          <button
            ...
            disabled={loading}
            className="flex-1 py-4 bg-primary-600 hover:bg-primary-700 disabled:bg-stone-300 text-white font-black uppercase tracking-widest text-sm rounded-xl shadow-lg shadow-primary-200 transition-all active:scale-95 flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
