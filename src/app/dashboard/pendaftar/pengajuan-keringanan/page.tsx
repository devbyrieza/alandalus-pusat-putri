...
export default function PengajuanKeringananPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("alandalus_alimam_keringanan_draft");
        if (saved) {
        ...
        console.warn("Failed to load keringanan draft:", e);
      }
    }
  }, []);
    ...
    if (typeof window !== "undefined") {
      try {
        localStorage.setItem(
          "alandalus_alimam_keringanan_draft",
          JSON.stringify({ kesanggupanBayar, alasan })
        );
      } catch (e) {
        console.warn("Failed to save keringanan draft:", e);
      }
    }
