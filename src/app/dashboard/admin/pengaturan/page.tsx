...
export default function PengaturanPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("pengaturan_form_draft");
      if (saved) {
        try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const t = setTimeout(() => {
        localStorage.setItem("pengaturan_form_draft", JSON.stringify(editFormData));
      }, 500);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (typeof window !== "undefined") {
          localStorage.removeItem("pengaturan_form_draft");
        }
        setEditingTa(null);
        fetchTahunAjaran();
      } else {
  ...
  return (
    <div className="space-y-6">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="mb-6">
              ...
              {activeTahunAjaran ? (
                <div className="bg-gradient-to-r from-green-50 to-primary-50 border-2 border-green-200 rounded-xl p-4 flex items-center justify-between">
            ...
            {!has2027 && (
              <div className="mb-6">
                <button
                  ...
                  disabled={seeding}
                  className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-primary-600 to-indigo-600 hover:from-primary-700 hover:to-indigo-700 text-white rounded-xl font-bold transition-all disabled:opacity-50"
                >
                  {seeding ? (
      ...
      {/* Edit Modal Overlay */}
      {editingTa && (
        <div onWheel={(e) => e.stopPropagation()} data-modal="true" aria-modal="true" role="dialog" className="fixed inset-0 z-[9999] flex items-start md:items-center pt-10 md:pt-0 pb-20 md:pb-0 justify-center bg-stone-900/50  p-4 overflow-y-auto overflow-x-hidden overscroll-contain custom-scrollbar">
            ...
            <form onSubmit={handleUpdate} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-xl border-2 border-stone-200 focus:border-primary-500 outline-none transition-all"
                  required
                />
              </div>
              ...
              <div className="flex items-center gap-3 py-2">
                <input
                  ...
                  className="w-5 h-5 rounded border-2 border-stone-300 text-primary-600 focus:ring-primary-500"
                />
                <label
                  htmlFor="is_active"
              ...
              <div className="pt-4 flex gap-3">
                <button
                  type="button"
                  onClick={() => setEditingTa(null)}
                  className="flex-1 px-6 py-3 bg-stone-100 hover:bg-stone-200 text-stone-600 font-bold rounded-xl transition-all whitespace-nowrap inline-block shrink-0"
                >
                  Batal
                </button>
                <button
