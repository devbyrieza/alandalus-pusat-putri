...
export default function PendaftarDetailPage() {
  ...
  // Welcome Day Edit State
  const [isWdModalOpen, setIsWdModalOpen] = useState(false);
  const [wdFormData, setWdFormData] = useState({
  ...
  const handleUbahJenjang = async () => {
    ...
    const { value: newJenjang } = await Swal.fire({
      ...
      html: "<div class='text-left text-sm mb-4 text-stone-600 bg-amber-50 p-3 rounded-xl border border-amber-200'>" +
    ...
    if (selectedKeringanan) {
      ...
      if (selectedKeringanan === "none") {
        ...
        const { value: formValues } = await Swal.fire({
          ...
          html: `
            <div class="text-left text-sm mb-4 text-stone-600 bg-amber-50 p-3 rounded-xl border border-amber-200">
              Masukkan nominal <b>POTONGAN (Diskon)</b> (bukan sisa tagihan).<br/><br/>
              Contoh: Jika Uang Pangkal normal Rp 35.000.000, dan wali santri hanya sanggup bayar Rp 25.000.000, berarti <b>potongannya adalah Rp 5.000.000</b>. <br/>Ketik: <b>1500000</b>.
            </div>
  ...
  const handleSaveWd = async () => {
    setSavingWd(true);
    try {
      const res = await fetch(`/api/admin/pendaftar/${params.id}/welcome-day`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(wdFormData) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Gagal menyimpan");
      
      Swal.fire("Berhasil", "Data Welcome Day berhasil diperbarui", "success");
    ...
    return (
      <div className="text-center py-20">
        ...
        <Link
          ...
          className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-xl"
        >
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Summary Card */}
      <div className="bg-linear-to-br from-primary-600 to-primary-900 rounded-xl shadow-lg shadow-primary-900/20 p-5 md:p-8 text-white relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 relative z-10">
          ...
          <div className="flex items-center gap-5">
            <div className="w-20 h-20 bg-white/20  rounded-xl flex items-center justify-center text-gold-300 shadow-inner">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-2">
                <span className="font-mono bg-white/20 px-2.5 py-1 rounded-xl text-sm font-black text-white border border-white/20">
                  {pendaftar.nomor_pendaftaran}
                </span>
                <span className="px-2.5 py-1 bg-gold-400 text-primary-900 rounded-xl text-xs font-black uppercase shadow-sm">
                  {pendaftar.jenjang === "IL" ? "IL" : "SMP IT"}
                </span>
          ...
          <div className="flex flex-wrap gap-4">
            <div className="bg-white/10  rounded-xl px-5 py-4 min-w-[120px] border border-white/10">
              ...
              <p className="font-black text-xl flex items-center gap-1.5 text-white">
                {isEnrolled || hasPaidRegistration ? (
                  <>
                    <CheckCircle className="w-5 h-5 text-primary-400" /> Lunas
                  </>
                ) : hasPendingPayment ? (
                  <>
        ...
        <div className="mt-6 pt-6 border-t border-white/20 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span
              className={`px-3 py-1 rounded-xl text-sm font-bold ${statusInfo.color}`}
            >
              {statusInfo.label}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                Swal.fire({
                  ...
                  confirmButtonText: "Ya, buka kunci!" }).then(async (result) => {
                  if (result.isConfirmed) {
                    setSavingStatus(true);
                    try {
              ...
              className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-black transition-all flex items-center gap-1 disabled:opacity-50 shadow-sm active:scale-95"
            >
            ...
            {pendaftar.no_hp && (
              <a
                ...
                className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-black transition-all flex items-center gap-1 shadow-sm active:scale-95"
              >
            ...
            {userRole === "admin_super" && (
              <>
                <button
                  ...
                  className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-black transition-all flex items-center gap-1.5 shadow-sm active:scale-95 border border-primary-500 mr-2"
                >
                ...
                <button
                  ...
                  disabled={exporting}
                  className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-black transition-all flex items-center gap-1.5 shadow-sm active:scale-95 mr-2"
                >
            ...
            {pendaftar.email && (
              <a
                ...
                className="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-xl text-xs font-black transition-all flex items-center gap-1  active:scale-95"
              >
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="mb-4 grid grid-cols-1 md:grid-cols-2 sm:grid-cols-1 lg:grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    ...
                    className="px-3 py-2 bg-primary-50 hover:bg-primary-100 text-primary-700 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 border border-primary-100 disabled:opacity-50 whitespace-nowrap inline-block shrink-0"
                  >
          ...
          {(isPenguji ||
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gold-100">
              ...
              {userRole === "admin_super" && (
                <div className="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-xl flex flex-col sm:flex-row sm:items-center gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <a
                      ...
                      target="_blank"
                      className="px-3 py-1.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-[11px] font-bold transition-colors shadow-sm"
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-primary-50/50 p-4 rounded-xl border border-primary-100 flex flex-col justify-between">
                  <div>
                    <span className="block text-xs text-primary-600 font-bold uppercase tracking-wide mb-1">
                      Tes Al-Qur&apos;an
                    </span>
                    {!pendaftar.nilai_ujian ? (
                      <span className="text-sm font-bold text-stone-400 italic">
                ...
                {(pendaftar?.jenjang?.toLowerCase() || "").includes("ma") && (
                  <div className="bg-primary-50/50 p-4 rounded-xl border border-primary-100 flex flex-col justify-between">
                      ...
                      {!pendaftar.nilai_ujian ? (
                        ...
                        <div className="flex flex-col mt-2 space-y-1">
                          ...
                          {(pendaftar.nilai_ujian as any)?.detail_lisan_arab?.rekomendasi && (
                            <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded inline-block max-w-full break-words whitespace-normal ${(pendaftar.nilai_ujian as any).detail_lisan_arab.rekomendasi === "Diterima" ? "bg-primary-100 text-primary-700" : "bg-red-100 text-red-700"}`}>
                              {(pendaftar.nilai_ujian as any).detail_lisan_arab.rekomendasi}
                            </span>
                          )}
                        </div>
                    ...
                    {pendaftar.nilai_ujian?.catatan_lisan_arab && (
                      <div className="mt-3 text-[10px] text-stone-500 line-clamp-3 italic border-t border-primary-200/50 pt-2 leading-relaxed">
                        "`${pendaftar.nilai_ujian.catatan_lisan_arab}`"
                      </div>
                    )}
                  </div>
                ...
                <div className="bg-primary-50/50 p-4 rounded-xl border border-primary-100 flex flex-col justify-between">
                    ...
                    {!pendaftar.nilai_ujian ? (
                      ...
                      <div className="flex flex-col mt-2 space-y-1">
                        ...
                        {(pendaftar.nilai_ujian as any)?.detail_cawalsan?.rekomendasi && (
                          <span className="text-xs font-black text-primary-700 bg-primary-100 border border-primary-200 px-2 py-0.5 rounded uppercase tracking-widest inline-block max-w-full break-words whitespace-normal whitespace-nowrap inline-block shrink-0">
                            {(pendaftar.nilai_ujian as any).detail_cawalsan.rekomendasi}
                          </span>
                        )}
                      </div>
                  ...
                  {pendaftar.nilai_ujian?.catatan_ortu && (
                    <div className="mt-2 text-xs text-stone-600 line-clamp-2 italic border-t border-primary-200/50 pt-2">
              ...
              {isPenguji && (
                <div className="mt-4 flex justify-end">
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-xl hover:bg-primary-700 transition-colors shadow hover:shadow-md font-bold text-sm"
                  >
                ...
                <div className="mt-4 flex justify-end gap-2">
                  {userRole === "admin_super" && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-amber-500 text-white rounded-xl hover:bg-amber-600 transition-colors shadow hover:shadow-md font-bold text-sm"
                    >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-violet-600 text-white rounded-xl hover:bg-violet-700 transition-colors shadow hover:shadow-md font-bold text-sm"
                  >
          ...
          {/* Kehadiran Welcome Day (Tampil jika sudah accepted/enrolled) */}
          {(userRole === "admin_super" || userRole === "admin") && 
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-primary-100">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-black text-primary-950 tracking-tight leading-tight">
                    Kehadiran Welcome Day
                  </h3>
                  <p className="text-sm text-ink-300 font-medium tracking-tight">
                    Informasi kehadiran, jumlah rombongan, dan kendaraan.
                  </p>
                </div>
              </div>
              ...
              {!pendaftar.reservasi || pendaftar.reservasi.length === 0 ? (
                <div className="p-4 bg-stone-50 border border-stone-200 rounded-xl flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-primary-600 text-white rounded-xl text-xs font-black"
                  >
                    Input Data
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  {pendaftar.reservasi.map((res: any) => {
                    ...
                    return (
                      <div key={res.id} className="p-5 bg-primary-50/50 rounded-xl border border-primary-100 shadow-sm relative">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                          <div>
                            <span className="block text-[10px] text-primary-600 font-black uppercase tracking-widest mb-1">Status Kehadiran</span>
                            <span className={`px-2.5 py-1 rounded-xl text-xs font-black ${data.statusKehadiran === 'HADIR' ? 'bg-primary-100 text-primary-700' : 'bg-red-100 text-red-700'}`}>
                          ...
                          {data.statusKehadiran === 'HADIR' && (
                            <>
                              <div>
                                <span className="block text-[10px] text-primary-600 font-black uppercase tracking-widest mb-1">Rombongan</span>
                                <span className="text-sm font-black text-primary-950">{data.totalPengantar || 0} Orang</span>
                                <span className="block text-xs text-primary-700">Ruang Utama: {(Number(data.jumlahPendamping) || 0) + 1} Org</span>
                              </div>
                              <div>
                                <span className="block text-[10px] text-primary-600 font-black uppercase tracking-widest mb-1">Kendaraan</span>
                                <span className="text-sm font-black text-primary-950">{data.jumlahMobil || 0} Mobil, {data.jumlahMotor || 0} Motor</span>
                              </div>
                            </>
                          )}
                          {data.catatanTambahan && (
                            <div className="sm:col-span-2 md:col-span-3">
                              <span className="block text-[10px] text-primary-600 font-black uppercase tracking-widest mb-1">Catatan</span>
                              <span className="text-xs text-primary-950">{data.catatanTambahan}</span>
                            </div>
                          )}
                        </div>
                        <div className="mt-4 flex justify-end">
                          <button
                            onClick={() => {
                              setWdFormData({
                                statusKehadiran: data.statusKehadiran || "HADIR",
                                jumlahPendamping: data.jumlahPendamping || 1,
                                totalPengantar: data.totalPengantar || 3,
                                catatanTambahan: data.catatanTambahan || "",
                                jumlahMobil: data.jumlahMobil || 0,
              ...
              {pendaftar.pembayaran.length === 0 ? (
                ...
                <div className="space-y-4">
                  {pendaftar.pembayaran.map((payment) => (
                    <div
                      key={payment.id}
                      className="p-4 bg-gold-50/50 rounded-xl border border-gold-100 shadow-xs"
                    >
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          {(!payment.bukti_transfer_path || payment.status_pembayaran !== "verified") && (
                            <button
                              ...
                              className="px-3 py-1 bg-white hover:bg-gold-100 text-gold-700 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all flex items-center gap-1.5 border border-gold-200 disabled:opacity-50"
                            >
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gold-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              />
              <InfoItem label="Jenjang" value={pendaftar.jenjang === "IL" ? "IL" : "SMP IT"} />
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 px-3 py-2 bg-stone-50 border border-stone-200 rounded-xl text-sm font-bold text-primary-950 focus:outline-none focus:ring-2 focus:ring-primary-500"
                  ...
                  <div className="flex items-end justify-between">
                    ...
                    {userRole === "admin_super" && (
                      <button
                        ...
                        className="p-1.5 text-primary-500 hover:text-primary-700 bg-primary-50 hover:bg-primary-100 rounded-xl transition-colors flex items-center gap-1 whitespace-nowrap inline-block shrink-0"
                        title="Edit Nomor HP"
                      >
                        <Edit className="w-3.5 h-3.5" />
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 border-2 border-red-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="p-3 bg-red-50 text-red-900 rounded-xl border border-red-100 min-h-[60px]">
                    {pendaftar.hobi ||
        ...
        <div className="space-y-6">
          ...
          {!isKeuangan && (
            <div className="bg-white rounded-xl shadow-lg p-6 border-2 border-primary-100">
              <div className="flex items-center gap-3 mb-4">
                <div className="p-2 bg-primary-100 rounded-xl whitespace-nowrap inline-block shrink-0">
                  <FileText className="w-6 h-6 text-primary-700" />
                </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border-2 border-primary-100">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2 bg-stone-100 rounded-xl whitespace-nowrap inline-block shrink-0">
      ...
      {isEditModalOpen && editFormData && (
        <div onWheel={(e) => e.stopPropagation()} data-modal="true" aria-modal="true" role="dialog" className="fixed inset-0 bg-black/60  flex items-start md:items-center pt-10 md:pt-0 pb-20 md:pb-0 justify-center z-50 p-4 overflow-y-auto overflow-x-hidden overscroll-contain custom-scrollbar">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden border border-stone-200">
            {/* Modal Header */}
            ...
            <form onSubmit={handleSaveEdit} className="flex-1 overflow-y-auto p-5 md:p-8 space-y-6 overscroll-contain custom-scrollbar">
              ...
              <div className="border-t border-stone-200 pt-6 flex items-center justify-end gap-3 sticky bottom-0 bg-white">
                <button
                  type="button"
                  onClick={() => setIsEditModalOpen(false)}
                  className="px-6 py-3 bg-stone-100 hover:bg-stone-200 text-stone-800 rounded-xl font-black text-sm uppercase transition-all whitespace-nowrap inline-block shrink-0"
                >
                  Batal
                </button>
                <button
      ...
      {isNilaiModalOpen && (
        <div onWheel={(e) => e.stopPropagation()} data-modal="true" aria-modal="true" role="dialog" className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overscroll-contain">
          ...
          />
          <div className="relative w-full max-w-md bg-white rounded-[2rem] shadow-lg flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
      ...
      {isWdModalOpen && (
        <div onWheel={(e) => e.stopPropagation()} data-modal="true" aria-modal="true" role="dialog" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-primary-950/40  overflow-y-auto overscroll-contain custom-scrollbar">
          <div className="bg-white w-full max-w-lg rounded-xl shadow-lg flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-5 overscroll-contain custom-scrollbar">
                ...
                <div className="flex gap-4">
                  <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all ${
                    ...
                    <input
                      type="radio"
                      className="sr-only"
                      checked={wdFormData.statusKehadiran === "HADIR"}
                      onChange={() => setWdFormData({ ...wdFormData, statusKehadiran: "HADIR" })}
                    />
                  ...
                  <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all ${
                    ...
                    <input
                      type="radio"
                      className="sr-only"
                      checked={wdFormData.statusKehadiran === "TIDAK_HADIR"}
                      onChange={() => setWdFormData({ ...wdFormData, statusKehadiran: "TIDAK_HADIR" })}
                    />
                    <X className="w-5 h-5" />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-1 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-3 rounded-xl border border-surface-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/20 outline-none transition-all font-medium text-ink-600 bg-white"
                      />
                    </div>
                    <div>
                      <label className="text-xs font-black uppercase tracking-wider text-ink-300 mb-2 block">
                        Pendamping (Ruang Utama)
                      </label>
                      <select
              ...
              <div>
                ...
                <textarea
                  value={wdFormData.catatanTambahan}
                  onChange={(e) => setWdFormData({ ...wdFormData, catatanTambahan: e.target.value })}
                  placeholder="Opsional..."
                  rows={3}
                  className="w-full p-3 rounded-xl border border-surface-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/20 outline-none transition-all font-medium text-ink-600 bg-white resize-none"
                />
              </div>
            </div>

            <div className="p-6 border-t border-surface-100 bg-surface-50 rounded-b-3xl flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setIsWdModalOpen(false)}
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-black shadow-lg shadow-primary-500/30 transition-all active:scale-95 disabled:opacity-50"
              >
