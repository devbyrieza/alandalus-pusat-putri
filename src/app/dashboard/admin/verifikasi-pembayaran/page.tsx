    ...
    try {
      ...
      rawData.forEach((item: Pembayaran) => {
        ...
        if (cleanMetode.toLowerCase() === "manual") {
          cleanMetode = "Transfer Manual BSI Al-Andalus Putri";
        }
      ...
      const data = Object.values(groups).map((g) => {
        ...
        if (activeTab === "UANG_PANGKAL" || activeTab === "DAFTAR_ULANG") {
          expectedTagihan = 30000000 - g.beasiswa;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[2rem] shadow-sm p-5 md:p-8 border border-stone-100">
        <div className="flex flex-col xl:flex-row xl:items-center gap-4 xl:justify-between flex-wrap">
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-4">
            <div className="w-12 h-12 md:w-16 md:h-16 bg-primary-50 rounded-xl flex items-center justify-center border border-primary-100 shrink-0">
          ...
          <div className="flex flex-wrap bg-stone-100 p-1.5 rounded-[1.25rem] w-full sm:w-fit shadow-inner ring-1 ring-stone-200/50 justify-center whitespace-nowrap inline-block shrink-0">
            ...
            >
              ...
              {counts.PENDAFTARAN > 0 && (
                <span className="bg-primary-500 text-white flex items-center justify-center w-5 h-5 text-[10px] rounded-full shrink-0 shadow-sm">
                  {counts.PENDAFTARAN}
                </span>
              )}
            </button>
      ...
      <div className="relative group">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-4 bg-white border border-stone-200 rounded-xl focus:border-primary-500 focus:ring-4 focus:ring-primary-500/5 focus:outline-none transition-all text-sm md:text-base font-bold text-stone-800 placeholder:text-stone-400 shadow-sm"
        />
      </div>

      {/* Stats / Filter Bar */}
      <div className="flex flex-wrap items-center gap-3 bg-white p-4 rounded-xl shadow-sm border border-stone-100">
        <div className="px-4 py-2 bg-stone-100 rounded-xl text-sm font-bold text-stone-600 whitespace-nowrap inline-block shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-3 md:px-4 py-2 rounded-xl text-sm font-bold transition-all ${
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
                  <div className="flex-1">
                    ...
                    {/* Details Grid */}
                    <div className="grid grid-cols-1 sm:grid-cols-1 gap-4 ml-16 bg-stone-50/50 p-4 rounded-xl border border-stone-100">
                      <div>
                      ...
                      {pay.tipe_cicilan === "CICILAN" && (
                        <div>
                          ...
                          <span className="px-2.5 py-1 bg-violet-100 text-violet-700 rounded-xl text-[10px] font-black uppercase tracking-widest border border-violet-200 whitespace-nowrap inline-block shrink-0">
                      ...
                      {pay.keringanan_reason && (
                        <div className="col-span-1 sm:col-span-2 pt-2 border-t border-secondary-200/50 mt-1">
                          <p className="text-xs text-secondary-700 bg-secondary-50 p-2 rounded-xl border border-secondary-100 font-medium">
                  ...
                  <div className="flex sm:flex-col gap-3 sm:w-48 shrink-0">
                    ...
                    {statusFilter === "verified" && (
                      <button
                        onClick={() => openModal(pay)}
                        className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-3 bg-primary-50 hover:bg-primary-100 text-primary-700 rounded-xl font-bold text-sm border border-primary-100 transition-all whitespace-nowrap inline-block shrink-0"
                      >
                        <CheckCircle className="w-4 h-4" />
                        Terverifikasi
                      </button>
      ...
      {showModal && selectedPembayaran && (
        <div onWheel={(e) => e.stopPropagation()} data-modal="true" aria-modal="true" role="dialog" className="fixed inset-0 bg-black/60  flex items-start md:items-center pt-10 md:pt-0 pb-20 md:pb-0 justify-center z-50 p-4 animate-in fade-in duration-200 overflow-y-auto overflow-x-hidden overscroll-contain custom-scrollbar">
          <div className="bg-white rounded-[2rem] shadow-lg max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            {/* Header - Fixed */}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 overscroll-contain custom-scrollbar">
              ...
              {activeTab === "DAFTAR_ULANG" && (
                <div className="grid grid-cols-1 md:grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-violet-50 rounded-xl border border-violet-100">
                  <div>
              ...
              {selectedPembayaran.keringanan_reason && (
                <div className="p-5 bg-secondary-50 border-2 border-secondary-100 rounded-xl space-y-3 shadow-sm">
              ...
              {selectedPembayaran.bukti_transfer_url ? (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-primary-50 text-primary-600 hover:bg-primary-100 rounded-xl text-xs font-bold transition-colors disabled:opacity-50 whitespace-nowrap inline-block shrink-0"
                    >
                  ...
                  <div className="border-2 border-stone-100 rounded-xl overflow-hidden bg-stone-50 relative group">
                    {selectedPembayaran.bukti_transfer_url
                      ?.toLowerCase()
                      .endsWith(".pdf") ? (
                      <iframe
                    ...
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <a
                        ...
                        className="px-6 py-3 bg-white rounded-full font-bold text-stone-900 shadow-lg hover:scale-105 transition-transform flex items-center gap-2"
                      >
                        <Eye className="w-4 h-4" />
                        Lihat Full Size
                      </a>
                ...
                <div className="p-5 md:p-8 border-2 border-dashed border-stone-200 rounded-xl text-center space-y-4">
                  <p className="text-stone-500 font-medium">
                  ...
                  <button
                    onClick={handleUploadClick}
                    disabled={uploadingProof === selectedPembayaran.id}
                    className="mx-auto flex items-center justify-center gap-2 px-4 py-2 bg-primary-50 text-primary-600 hover:bg-primary-100 rounded-xl text-sm font-bold transition-colors disabled:opacity-50 whitespace-nowrap inline-block shrink-0"
                  >
                    {uploadingProof === selectedPembayaran.id ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ) : (
            ...
            <div className="p-6 border-t border-stone-100 bg-stone-50/50 flex flex-col sm:flex-row gap-3">
              <div className="flex-1 flex gap-3">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-4 bg-primary-500 hover:bg-primary-600 text-white rounded-xl font-bold transition-all hover:shadow-lg hover:shadow-primary-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {processing ? (
              ...
              {selectedPembayaran.status_pembayaran !== "pending" && (
                <button
                  ...
                  disabled={processing}
                  className="px-6 py-4 bg-stone-100 hover:bg-stone-200 text-stone-600 rounded-xl font-bold transition-all disabled:opacity-50 border border-stone-200 whitespace-nowrap inline-block shrink-0"
                >
                  {processing ? (
