    ...
    <div className="min-h-screen bg-[#fafaf9] font-sans selection:bg-primary-100 selection:text-primary-900">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-[60] bg-white backdrop-blur-2xl border-b border-ink-200/60 px-6 py-4 flex items-center justify-between shadow-sm">
        <button
          ...
          className="p-2.5 -ml-2 text-ink-600 hover:bg-ink-100 rounded-xl transition-all active:scale-95"
        >
        ...
        <div className="w-10 h-10 rounded-xl bg-linear-to-br from-primary-700 to-primary-900 flex items-center justify-center text-white text-sm font-black shadow-lg shadow-primary-200">
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed top-0 left-0 bottom-0 z-[80] w-80 bg-white shadow-lg lg:hidden flex flex-col rounded-r-[3rem] overflow-hidden"
            >
              <div className="p-5 md:p-8 flex items-center justify-between border-b border-ink-50 bg-linear-to-b from-ink-50 to-white">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-clay-sm border border-ink-100">
                    <img
                ...
                <button
                  onClick={() => setSidebarOpen(false)}
                  className="p-3 hover:bg-red-50 rounded-xl text-red-400 transition-all active:scale-90"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                {menuItems.map((item) => (
                  <Link
                    ...
                    className={`flex items-center justify-between px-4 py-4 rounded-xl transition-all ${
                      item.isActive
                        ? "bg-primary-700 text-white shadow-lg shadow-primary-200 font-bold"
              ...
              <div className="p-5 md:p-8 border-t border-ink-50 space-y-3">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-red-50 text-red-600 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-red-100 transition-all active:scale-95"
                >
      ...
      >
        ...
        <div className="h-24 flex items-center px-5 md:px-8">
          ...
          >
            <div className="shrink-0 w-12 h-12 bg-white rounded-xl flex items-center justify-center border border-ink-100 shadow-lg group-hover:rotate-6 transition-all duration-500">
              <img
                src={BRANDING.logoPath}
                alt="Logo"
            ...
            {menuItems.map((item, idx) => {
              ...
              return (
                <div key={item.name} className="relative">
                  ...
                  <Link
                    href={item.href}
                    className={`relative group flex items-center justify-between px-4 py-3.5 rounded-xl transition-all duration-300 ${
                      item.isActive
                  ...
                  >
                    ...
                    {item.isActive && (
                      <motion.div
                        ...
                        className="absolute inset-0 bg-primary-50 rounded-xl -z-10 border border-primary-100 shadow-sm"
                        transition={{
                          type: "spring",
                          bounce: 0.2,
                          duration: 0.6 }}
                    ...
                    {collapsed && (
                      <div className="absolute left-full ml-6 px-4 py-2 bg-primary-950 text-white text-[11px] font-black rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-[100] shadow-lg border border-white/10 uppercase tracking-widest">
                        {item.name}
                      </div>
                    )}
                  </Link>
        ...
        <div className="p-6 border-t border-ink-50 bg-ink-50/30">
          ...
          {/* Profile Card */}
          <div
            className={`flex items-center gap-4 p-3 rounded-xl transition-all group relative ${collapsed ? "justify-center" : "bg-white shadow-sm border border-ink-100"}`}
          >
            <div className="shrink-0 w-10 h-10 rounded-xl bg-linear-to-br from-primary-700 to-primary-900 flex items-center justify-center text-white font-black text-sm shadow-lg shadow-primary-100 ring-4 ring-white border border-primary-500/20">
              {(adminName || "A").charAt(0)}
            </div>
            {!collapsed && (
      ...
      >
        ...
        <header className="hidden lg:flex sticky top-0 z-40 bg-white backdrop-blur-2xl border-b border-ink-100 h-24 items-center justify-between px-6 md:px-12">
          ...
          <div className="flex items-center gap-6">
            ...
            <button className="relative p-3 text-ink-400 hover:text-primary-700 hover:bg-primary-50 rounded-xl transition-all active:scale-95">
            ...
            <Link
              href="/"
              className="px-5 md:px-8 py-3.5 bg-primary-950 text-white text-[11px] font-black uppercase tracking-[0.2em] rounded-xl hover:bg-primary-800 transition-all shadow-lg shadow-primary-950/20 active:scale-95"
            >
              Kunjungi Situs
            </Link>
          </div>
